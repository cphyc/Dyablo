...
void GhostCommunicator_partial_blocks::init( const AMRmesh_hashmap_new& amr_mesh, const ForeachCell::CellArray_global_ghosted::Shape_t& shape, uint32_t ghost_count, const MpiComm& mpi_comm )
{
  ...
  this->m_local_intermediate_ghost_octants = 0;
}


// Cannot modify directly amr_mesh in AMRmesh_hashmap_new because no compatibility with other backends
// Thus, we need to output directly the intermediate views in a NEW ghostmap, instead of modifying the attributes of amr_mesh.
// Copied from init, then modified for intermediate case
void GhostCommunicator_partial_blocks::init_intermediates( 
  ...
  const MpiComm& mpi_comm )
{
  ...
  // Exchange ghostmap sizes
  //GhostMap_t ghostmap = amr_mesh.getGhostMap();
  GhostMap_t ghostmap = amr_mesh.getGhostMapIntermediate();
  int mpi_size = mpi_comm.MPI_Comm_size();
  ...
  this->m_send_intermediate_cell_count = send_sizes;
  this->m_recv_intermediate_cell_count = recv_sizes;
  this->m_send_intermediate_iOct = send_iOct;
  this->m_send_intermediate_iCell = send_iCell; 
  this->m_recv_intermediate_iOct = recv_iOcts;
  this->m_recv_intermediate_iCell = recv_iCell;
  this->m_local_intermediate_ghost_octants = local_octant_count;
}
