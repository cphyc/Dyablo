...
#include "gravity/GravitySolver_multigrid.h"
...
std::shared_ptr<AMRmesh> mesh_amrgrid_semiperiodic_sphere()
{
  ...
  std::cout << "// Boundary conditions : (absorbing, absorbing, absorbing) \n";
  std::cout << "// =========================================\n";
...
{ 
  ...
  std::string configmap_str = 
    ...
    "[output]\n"
    "outputPrefix=test_GravitySolver_mg\n"
  ...
  GravitySolver_multigrid gravitysolver( 
    configMap,
    foreach_cell,
    timers
  );
  ...
  real_t rcore = 2*Hernquist::r0;
  {
    ...
    foreach_cell.reduce_cell( "Init", Uout.getShape(),
      KOKKOS_LAMBDA( const ForeachCell::CellIndex& iCell, int err_count )
    {
      ...
      real_t Phi_num = Uout.at( iCell, Iphi ) + (Phi_ana_mean-Phi_num_mean);
      ...
      Uout.at(iCell, Igx) = Phi_ana;
      Uout.at(iCell, Igy) = Phi_num;
      Uout.at(iCell, Igz) = Phi_err;
    }, err_count);
