      ...
      PATCH_LAMBDA( const ForeachCell::Patch& patch )
    {
        ...
        KOKKOS_LAMBDA( const CellIndex& iCell_Uout )
      {
        ...
        auto get_slope = [&](int level_diff_L, int level_diff_R, const CellIndex &iCell_Qpatch, ComponentIndex3D dir) 
        {
          ...
          const PrimState qL = policy.getPrimState(Qpatch, iCell_Qpatch.getNeighbor( -(dir==IX), -(dir==IY), -(dir==IZ), search_local, CellIndex::Status::LOCAL_TO_BLOCK )); 
        ...
        auto process_dir = [&](const CellIndex &iCell_Uin, const CellIndex &iCell_Qpatch, ComponentIndex3D dir) {
          ...
          ConsState fluxL {};
          {
            ...
            {
              const CellIndex iCell_Uin_m = iCell_Uin.getNeighbor<CellIndex::Status::BOUNDARY>(-(dir==IX), -(dir==IY), -(dir==IZ), search_neighbor);
            ...
            {  
              if (Ldiff >= 0) 
              {
                CellIndex iCell_Qpatch_m = iCell_Qpatch.getNeighbor<CellIndex::Status::LOCAL_TO_BLOCK>( -(dir==IX), -(dir==IY), -(dir==IZ), search_local ); 
                ...
                if (Ldiff == 1) 
                {
                  ...
                  const CellIndex iCell_Uin_m = iCell_Uin.getNeighbor<CellIndex::Status::BIGGER>(-(dir==IX), -(dir==IY), -(dir==IZ), search_neighbor);
          ...
          {      
            ...
            {
              const CellIndex iCell_Uin_p = iCell_Uin.getNeighbor<CellIndex::Status::BOUNDARY>( (dir==IX),  (dir==IY),  (dir==IZ), search_neighbor);
            ...
            else
            {
              if (Rdiff >= 0) 
              {
                CellIndex iCell_Qpatch_p = iCell_Qpatch.getNeighbor<CellIndex::Status::LOCAL_TO_BLOCK>(  (dir==IX),  (dir==IY),  (dir==IZ), search_local ); 
                ...
                if (Rdiff == 1)
                {
                  ...
                  CellIndex iCell_Uin_p = iCell_Uin.getNeighbor<CellIndex::Status::BIGGER>( (dir==IX),  (dir==IY),  (dir==IZ), search_neighbor);
