...
public: 
  ...
  template< typename Array_t > void V_cycle_uniform(const Array_t& U, const Array_t& Uintermediate, const uint32_t level, const GhostCommunicator& ghost_comm);
  template< typename Array_t > void V_cycle_amr(const Array_t& U, const Array_t& Uintermediate, const uint32_t current_level, const uint32_t finest_level, const GhostCommunicator& ghost_comm);
  
  // Laplacian
  template< typename Array_t > void initialise_lhs(const Array_t& U, const Array_t& Uintermediate, const uint32_t level);
  template< typename Array_t > real_t residual_norm(const Array_t& U, const Array_t& Uintermediate, const uint32_t level);
  ...
  template< typename Array_t > void smoothing_intermediate_amr_correction(const Array_t& Uintermediate, const uint32_t nIterations, const uint32_t level, const GhostCommunicator& ghost_comm);
  template< typename Array_t > void smoothing_uniform(const Array_t& U, const Array_t& Uintermediate, const uint32_t nIterations, const uint32_t level, const GhostCommunicator& ghost_comm);                 
  template< typename Array_t > void smoothing_amr_finest(const Array_t& U, const Array_t& Uintermediate, const uint32_t nIterations, const uint32_t level, const GhostCommunicator& ghost_comm);
  struct Data;
private:
